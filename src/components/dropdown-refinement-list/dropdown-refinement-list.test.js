...
describe('DropdownRefinementList', () => {
    ...
    it('allows multiple selection of the dropdown options', () => {
        const mockProps = {...defaultProps, title: 'Test', items: mockRefinementFilterItems};
        render(<DropdownRefinementListComponent {...mockProps} />)
    ...
    it('shows the selection option count in the dropdown button', () => {
        const mockProps = {...defaultProps, title: 'Test', items: [{...mockRefinementFilterItems[0], isRefined: true}]};
        render(<DropdownRefinementListComponent {...mockProps} />)
    ...
    it('closes the dropdown if clicked outside', () => {
        const mockProps = {...defaultProps, title: 'Test', items: mockRefinementFilterItems};
        render(<div data-testid="outside-wrapper"><DropdownRefinementListComponent {...mockProps} /></div>)

        fireEvent.click(screen.getByText(mockProps.title))

        expect(screen.getByTestId('refinement-list').children).toHaveLength(mockProps.items.length)
